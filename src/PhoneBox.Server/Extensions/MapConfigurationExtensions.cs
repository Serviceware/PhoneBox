...
namespace Microsoft.Extensions.DependencyInjection
{
    ...
    {
        public static IConfigurationMappingExpression<TOptions> ConfigureTarget<TOptions>(this IServiceCollection services, string name, IConfiguration configuration) where TOptions : class
        {
            if (services == null)
        ...
        private sealed class ConfigurationMappingExpression<TToOptions> : IConfigurationMappingExpression<TToOptions> where TToOptions : class
        {
            ...
            public ConfigurationMappingExpression(IConfiguration configuration)
            {
                ...
                this._mappings = new Dictionary<Func<IServiceProvider, object?>, Delegate>();
            }
            ...
            {
                this._mappings.Add(x => this.ResolveOptionsValue<TFromOptions>(x, configurationName), mapper);
                return this;
            }
            ...
            private TFromOptions ResolveOptionsValue<TFromOptions>(IServiceProvider serviceProvider, string configurationName) where TFromOptions : new()
            {
                ...
                return this._configuration.Bind<TFromOptions>(configurationName);
            }

            private void Map(IServiceProvider serviceProvider, TToOptions to)
            {
                foreach (KeyValuePair<Func<IServiceProvider, object?>, Delegate> mapping in this._mappings)
                {
